...
/**@author Tomy
 * Created by Tomy on 31/8/2021.
 */
class HttpTokenInterceptor @JvmOverloads constructor(
    private val logger: Logger = Logger.DEFAULT, val mOnTokenRefreshListener: OnTokenRefreshListener? = null
) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (response.code == ErrorCode.TOKEN_INVALID) {
            ...
            if (!token.isNullOrEmpty()) {
                mOnTokenRefreshListener?.onTokenRefresh(token)
//                val newRequest = request.newBuilder().header("token", token).build()
                response.body?.close()
                response = chain.proceed(request)
